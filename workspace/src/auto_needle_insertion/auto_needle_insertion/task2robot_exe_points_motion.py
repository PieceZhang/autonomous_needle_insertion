...
class TaskProcedurePublisher(rclpy.node.Node):
    ...
    def __init__(self, topic_name: str = "task_procedure") -> None:
        super().__init__("task2_motion_task_procedure_pub")
...
class TaskInfoParamsPublisher(rclpy.node.Node):
    ...
    def __init__(self, *, topic_name: str = "/task_info", hz: float = 1.0, payload: Optional[dict] = None) -> None:
        super().__init__("task2_motion_params_publisher")
        self._pub = self.create_publisher(String, topic_name, 10)
    ...
    def _timer_cb(self) -> None:
        base_payload = {
            "TASK_NAME": "task2robot_exe_points_motion",
...
# ----------------- Parameters -----------------
NODE_NAME = "task2_exe_points_motion"
...
INITIAL_AREA_DIAMETER = 0.12  # meters, used only for jitter fallback
RAND_ROT_DEG = 10.0
MAXIMUM_TRACKER_LOST = 5
DEFAULT_TARGET_IDS = ["1", "2", "3"]

def _resolve_target_choices() -> List[str]:
    env_raw = os.getenv("TARGET_P", "").strip()
    if not env_raw:
        return DEFAULT_TARGET_IDS.copy()
    choices = [c.strip() for c in env_raw.split(",") if c.strip()]
...
class ExecPointsMotion:
    ...
    def loop_once(self, target_to_in_tracker: np.ndarray) -> bool:
        ...
        # Step 2: operator starts URCaps
        self.task_proc_pub.publish_step("2")
        if not _wait_for_enter(self, "Start URCaps on panel, then press Enter (or 'c' to cancel)..."):
            return False

        # Step 3: auto move to target
        ...
        # Step 4: operator stops URCaps
        self.task_proc_pub.publish_step("4")
        if not _wait_for_enter(self, "Stop URCaps on panel, then press Enter (or 'c' to cancel)..."):
            return False
        ...
        try:
            ...
            # Step 6: manual action
            self.task_proc_pub.publish_step("6")
            if not _wait_for_enter(self, "Drive the robot arm to perform the action, then press Enter to stop recording (or 'c' to cancel)..."):
                return False
        finally:
            if recording_started and not rclpy.ok():
...
# ----------------- Key wait helpers -----------------
def _wait_for_enter(task: ExecPointsMotion, prompt: str, allow_cancel: bool = True) -> bool:
...
def _cancel_requested(task: ExecPointsMotion) -> bool:
    while True:
        token = task.key_input.get_key()
        if token is None:
            return False
...
def _get_target_from_env() -> str:
    ...
    if not TARGET_CHOICES:
        raise RuntimeError("TARGET_CHOICES is empty; set TARGET_P or use default targets 1,2,3")
    if not env_raw or "," in env_raw:
        return TARGET_CHOICES[0]
    if env_raw not in TARGET_CHOICES:
        raise RuntimeError(f"TARGET_P must be one of {TARGET_CHOICES}, got '{env_raw}'")
    return env_raw
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Execute task2 motion loop to recorded points")
...
def main() -> None:
    ...
    task = ExecPointsMotion(target_name, targets)
    try:
        print("Looping steps 2-7; press 'c' to cancel at any prompt.", flush=True)
        while rclpy.ok():
            ...
            try:
                ...
                break
        print("Exiting task2 motion loop.", flush=True)
    finally:
        task.close()
