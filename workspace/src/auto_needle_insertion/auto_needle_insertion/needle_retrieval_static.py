...
# --- Inserted function ---
def read_needle_pose_as_quaternion(
    topic: str = "/ndi/needle_pose",
    timeout_sec: float = 2.0,
    node: Optional[Node] = None,
    qos_depth: int = 1,
) -> Tuple[float, float, float, float, float, float, float]:
    """Read a single needle pose message and return it as a position+quaternion tuple.

    This helper subscribes to `topic` expecting a `geometry_msgs/msg/PoseStamped` and
    waits until one message arrives (or times out). The returned pose is:
    ...
    if node is None:
        node = rclpy.create_node(f"{NODE_NAME}_needle_pose_reader")
        owns_node = True
