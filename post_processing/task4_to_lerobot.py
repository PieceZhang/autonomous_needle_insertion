...
def load_ndjson_optional(path: Path):
    """Return (times, payloads) or (None, None) if missing/unreadable/empty."""
    try:
        if not path.exists():
            return None, None
...
def load_video_stream_optional(episode_dir: Path, folder: str, max_frames=None):
    """
    Return dict:
      {"present": bool, "frames": list[np.ndarray], "times_ns": np.ndarray, "info": dict}
    ...
    if not info_p.exists() or not vid_p.exists():
        return {"present": False, "frames": None, "times_ns": None, "info": None}
    ...
    try:
        ...
        return {"present": True, "frames": frames, "times_ns": times_ns, "info": info}
    except Exception:
        return {"present": False, "frames": None, "times_ns": None, "info": None}
...
def convert_one_episode(
    ...
    if not wrgb_stream["present"]:
        raise RuntimeError(f"{episode_dir.name}: missing wrist rgb stream folder/video: {folder_wrgb}")
    ...
    room_fps = (room_stream["info"]["fps"] if room_stream["present"] and room_stream["info"]["fps"] > 1e-3 else 0.0)

    print(
        f"[INFO] {episode_dir.name}: time-align streams. "
        f"room_present={room_stream['present']}, room_fps={room_fps:.3f}, "
        f"us_fps={us_fps:.3f}, wrgb_fps={wrgb_fps:.3f}, wdepth_fps={wdepth_fps:.3f}, "
    ...
    return T, {
        ...
        "room_present": bool(room_stream["present"]),
        "wrench_present": bool(wrench_present),
...
def main():
    ...
    parser.add_argument("--repo_id", type=str, default="local/task41_needle_retrieval_v1")
    ...
    features = lab_features_schema_task41_fixed_hw()
